...
export default function Accounting() {
  ...
  const createCustomerMutation = useMutation({
    mutationFn: (data: any) => apiRequest('/api/accounting/customers', 'POST', JSON.stringify(data)),
    onSuccess: () => {
  ...
  const createVendorMutation = useMutation({
    mutationFn: (data: any) => apiRequest('/api/accounting/vendors', 'POST', JSON.stringify(data)),
    onSuccess: () => {
  ...
  const generateProfitLossMutation = useMutation({
    mutationFn: (data: { startDate: string; endDate: string }) => 
      apiRequest('/api/accounting/reports/profit-loss', 'POST', JSON.stringify(data)),
    onSuccess: () => {
  ...
  const generateBalanceSheetMutation = useMutation({
    mutationFn: (data: { asOfDate: string }) => 
      apiRequest('/api/accounting/reports/balance-sheet', 'POST', JSON.stringify(data)),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/accounting/reports'] });
  ...
  if (!(effectiveAccessCheck as any)?.hasAccess) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <Card className="w-full max-w-md">
          ...
          <CardContent className="text-center">
            ...
            <p className="text-sm text-gray-600">
              Current user: {(effectiveAccessCheck as any)?.user || user?.username || 'Unknown'}
            </p>
